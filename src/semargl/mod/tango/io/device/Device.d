...
class Device: Conduit, ISelectable
{
	...
	version(Win32)
	{
		...
		override size_t read(byte* dst_ptr, ulong size)
		{
			DWORD read;
			void* p = dst_ptr;

			if(!ReadFile(handle, p, size, &read, null))
				// make Win32 behave like linux
				if(SysError.lastCode is ERROR_BROKEN_PIPE)
					return Eof;
				else
	...
	version(Posix)
	{
		...
		override size_t read(void[] dst)
		{
			...
			if(read == -1)
				error;
			else if(read is 0 && dst.length > 0)
				return Eof;
			return read;
		}
		...
		override size_t read(byte* dst_ptr, ulong size)
		{
			int read = posix.read(handle, dst_ptr, size);
			if(read == -1)
				error;
			else if(read is 0 && size > 0)
				return Eof;
			return read;
		}
